...
@description('Enable blob soft-delete on the storage account. Required for Azure AI Search native blob soft-delete deletion detection so the contracts knowledge source removes orphaned index documents when a blob is deleted (e.g. a cmo-/sup- rename).')
param enableBlobSoftDelete bool = true

@description('Retention (days) for blob soft-delete when enableBlobSoftDelete is true.')
@minValue(1)
@maxValue(365)
...
// Enable blob soft-delete so Azure AI Search native blob soft-delete deletion
// detection can reconcile deletions: when a contracts blob is deleted (e.g. a
// cmo-/sup- rename), it becomes soft-deleted and a normal indexer run removes
// the orphaned index document instead of leaving it stranded.

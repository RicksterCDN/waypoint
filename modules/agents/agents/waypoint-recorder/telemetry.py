...
def operation_id() -> str | None:
    """Return a stable per-execution operation id for idempotency/correlation.

    Prefers the current OpenTelemetry trace id (which App Insights surfaces as the
    operation id and which is shared across the orchestrator fan-out and this
    recorder call), then falls back to the APP_INSIGHTS_OPERATION_ID env value.
    Returns None when neither is available so callers can degrade to a per-call id.
    """
    ...
    env_value = os.getenv("APP_INSIGHTS_OPERATION_ID")

...
def _build_reply_card(reply_text: str):
    """Return (human_text, attachments) for a model reply.

    Extracts the structured card payload (if any) and renders an Adaptive Card
    attachment dict. On any problem it returns the original text and no
    attachment, so the caller always has a safe plain-text reply.
    """
    try:
        ...
        if payload is None:
            logger.warning(
                "Adaptive Card payload missing; sending generated fallback card."
            )
            card = build_text_fallback_card(reply_text)
            return "", [card_attachment_dict(card)]
        card = build_status_card(payload)
        # Card is self-contained; drop the Markdown body to avoid a duplicate
        # "wall of text" above the card. summary lives in the card header.
        return "", [card_attachment_dict(card)]
    except Exception:  # pragma: no cover - never let card rendering break a reply
        logger.exception("Adaptive Card rendering failed; sending plain text.")
        return reply_text, None


def _plain_reply(reply_text: str) -> str:
    """Human-readable Markdown for *reply_text* with any card payload stripped.

    Used as the safe fallback when a channel rejects the Adaptive Card: the
    approver still gets the model's written answer instead of an error.
    """
    try:
        from adaptive_cards import extract_card_payload

        human_text, _ = extract_card_payload(reply_text)
        return (human_text or "").strip()
...
def _mount_anonymous_activity_protocol(host: Any, agent: Any) -> None:
    ...
    async def _handle_anonymous_messages(request: Request) -> Response:
        ...
        # Split any structured card payload out of the model text and, when
        # present, attach a rendered Adaptive Card. Falls back to plain text.
        reply_text, card_attachments = _build_reply_card(reply_text)
        ...
        if service_url and conversation_id:
            ...
            if card_attachments:
                reply_activity["attachments"] = card_attachments
            try:
                post_url = (
...
def mount_activity_protocol(host: Any, agent: Any) -> None:
    ...
    async def _on_message(context: TurnContext, _state: TurnState) -> None:
        ...
        logger.warning("activity message received: %r", text[:120])
        if not text:
            return
        try:
            result = await agent.run(text)
        ...
        if not reply:
            return
        human_text, attachments = _build_reply_card(reply)
        if attachments:
            from microsoft_agents.activity import Activity, ActivityTypes, Attachment

            try:
                # NB: this SDK's Activity model requires ``text`` to be a string
                # — passing ``None`` raises a pydantic ValidationError at
                # construction. For card replies ``human_text`` is "" by design
                # (the card is self-contained), so coalesce to "" not None.
                card_activity = Activity(
                    type=ActivityTypes.message,
                    text=human_text or "",
                    attachments=[
                        Attachment(
                            content_type=att["contentType"], content=att["content"]
                        )
                ...
                # Never surface a bare error — fall back to the readable text
                # digest so the approver still gets an answer, and log why.
                logger.exception("Adaptive Card reply failed; sending text.")
                await context.send_activity(_plain_reply(reply) or reply)
        else:
            await context.send_activity(human_text)

    @AGENT_APP.error
    async def _on_error(context: TurnContext, error: Exception) -> None:
        logger.exception("Unhandled activity error: %s", error)
        try:

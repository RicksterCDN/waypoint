...
def add_span_event(span: Span, name: str, attributes: dict[str, Any] | None = None) -> None:
    ...
    for attr_name, value in (attributes or {}).items():
        if value is None:
            continue
        if _is_sensitive_attribute(attr_name):
            event_attributes[attr_name] = "**********"
        elif isinstance(value, (str, bool, int, float)):
            event_attributes[attr_name] = value
        else:
            event_attributes[attr_name] = json.dumps(value, sort_keys=True, default=str, separators=(",", ":"))
...
def trace_span(
    ...
    *,
    ...
    with trace.get_tracer("forge.hosted_agents").start_as_current_span(name, kind=kind) as span:

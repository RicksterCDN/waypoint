...
def _parse_json_lenient(raw_text: str) -> dict:
    """Parse model output as JSON while tolerating common wrappers.

    Some models occasionally return valid JSON followed by additional prose,
    or wrap JSON in markdown fences. This parser accepts those cases while
    preserving strict errors for truly invalid payloads.
    """
    text = (raw_text or "").strip()
    if not text:
        raise ValueError("systematization returned empty response text")

    try:
        payload = json.loads(text)
        if not isinstance(payload, dict):
            raise ValueError("systematization response JSON must be an object")
        return payload
    ...
    if fence_match:
        ...
        if not isinstance(payload, dict):
            raise ValueError("systematization fenced JSON must be an object")
        return payload

    # Finally, parse the first valid JSON value and ignore trailing text.
    ...
    if trailing:
        log.warning(
            "systematization returned trailing non-JSON text; ignoring %d trailing chars",
            len(trailing),
        )
    return payload
...
async def run_systematization(
    *,
    ...
    if not isinstance(payload, dict) or not payload:
        ...
        try:
            payload = _parse_json_lenient(response.text)

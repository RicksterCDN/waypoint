...
_AZURE_IDENTITY_MODULE: Any | None = None
_AZURE_TOKEN_PROVIDER: Any | None = None
...
def _get_azure_identity_module() -> Any | None:
    global _AZURE_IDENTITY_MODULE
    if _AZURE_IDENTITY_MODULE is None:
        try:
            _AZURE_IDENTITY_MODULE = importlib.import_module("azure.identity")
        except ModuleNotFoundError:
            _AZURE_IDENTITY_MODULE = False
    return _AZURE_IDENTITY_MODULE or None


def _get_azure_token_provider() -> Any | None:
    ...
    if _AZURE_TOKEN_PROVIDER is None:
        ...
        _AZURE_TOKEN_PROVIDER = azure_identity.get_bearer_token_provider(
            credential,
...
def _azure_provider_kwargs(model: str) -> dict[str, Any]:
    ...
    # Prefer explicit API key auth when provided. In CI this avoids
    # unintentionally using the workflow principal's Azure AD token,
    # which may not have OpenAI data-plane permissions.
    if api_key:
    ...
    if token_provider is not None:
        try:
            payload["azure_ad_token"] = token_provider()
            payload.pop("api_key", None)
            return payload
        except Exception as exc:
            log.debug("Falling back from Azure token auth to key auth: %s", exc)
...
async def generate(
    ...
    if resolved_options.web_search:
        ...
        payload.update(_azure_provider_kwargs(model))
